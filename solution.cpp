...
void Solution::combination_get(int index, int target, vector<int> &arr,
...
vector<int> Solution::get_combination(int target, vector<int> &den) {
  vector<int> result;
#ifdef DEBUG
  cout << "get_combination()"
       << "\n";
#endif
  for (int i = 0; i < den.size(); i++) {
#ifdef DEBUG
    cout << "denomination: " << den[i] << "\n";
#endif
    while (target > 0) {
#ifdef DEBUG
      cout << "target :" << target << "\n";
      ...
      if ((target - den[i]) == 0 && (i == den.size() - 1)) {
        target -= den[i];
        result.push_back(den[i]);
      }
      break;
